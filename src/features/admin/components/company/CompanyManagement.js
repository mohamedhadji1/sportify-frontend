...
import { Building2, ChevronDown, PlusCircle } from 'lucide-react'; // Added PlusCircle
...
const CompanyManagement = () => {
  ...
  const getCurrentUser = useCallback(() => { // Memoize to prevent re-renders if used in useEffect deps
    try {
  ...
  const fetchCompanies = useCallback(async () => {
    ...
    try {
      ...
      if (user.role !== 'Admin' && fetchedCompanies.length > 0) {
        ...
        // companies state is already set (either with data or an empty array).
        // AdminCompanyTable will handle an empty `companies` array by showing "No companies found."
        ...
        setCurrentCompany(null);
        setEditForm({});
        // setCompanies([]); // This is already handled by the Array.isArray check or the catch block
      }
    } catch (err) {
  ...
  const handleApproveCompany = async (companyId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`http://localhost:5001/api/companies/${companyId}/verify`, {}, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleSuspendCompany = async (companyId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`http://localhost:5001/api/companies/${companyId}/suspend`, {}, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.data && response.data.company) {
        const updatedCompany = response.data.company;
  ...
  // Admin View: Render AdminCompanyTable
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-10">
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl p-8">
          ...
          >
            <PlusCircle size={20} className="mr-2" /> Add Your First Company
          </button>
        </div>
        <ToastContainer toasts={toasts} removeToast={removeToast} />
      </div>
    ...
    <div className="p-4 sm:p-6 bg-slate-900 min-h-screen">
      ...
      {currentUser && currentUser.role !== 'Admin' && (
        <div className="mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          >
            <PlusCircle size={20} className="mr-2" /> Add New Company
          </button>
        </div>
      )}
        ...
        <div className="border-t border-b border-slate-700 my-4">
          ...
          >
            ...
            <div className={`p-1 rounded-full transition-all duration-300 ${showDetails ? 'bg-blue-500/20' : 'bg-slate-600/30'} group-hover:bg-blue-500/30`}>
              <ChevronDown size={14} className={`transition-all duration-300 ${showDetails ? 'rotate-180 text-blue-400' : 'text-slate-400'} group-hover:text-blue-300`} />
            </div>
          </button>
        </div>
      )}
