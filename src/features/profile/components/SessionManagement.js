...
const SessionManagement = () => {
  ...
  const fetchSessions = async () => {
    ...
    try {
      ...
      if (token) {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/auth/sessions`, {
  ...
  const terminateSession = async (sessionId) => {
    try {
      const token = localStorage.getItem("token")
      const response = await axios.delete(`${process.env.REACT_APP_API_URL}/auth/sessions/${sessionId}`, {
        headers: { "x-auth-token": token },
      })
  ...
  const terminateAllSessions = async () => {
    try {
      const token = localStorage.getItem("token")
      const response = await axios.delete(`${process.env.REACT_APP_API_URL}/auth/sessions`, {
        headers: { "x-auth-token": token },
      })
