...
export const PlayerSignIn = ({ onClose, onSwitchToManager, onSwitchToPlayerSignUp, onSwitchToPasswordReset, on2FARequired }) => {  const [email, setEmail] = useState("")
  ...
  const handleInitialLogin = async (e) => {
    ...
    setIsLoading(true);
    setError("");
    setVerificationError("");    if (!email || !password) {
    ...
    <div className="space-y-5 md:space-y-6">
      ...
      <form onSubmit={handleInitialLogin} className="space-y-5">
        ...
        />        <TextInput
          ...
          disabled={isLoading}
          required
          icon={<Icons.Lock className="h-5 w-5" />}          showPasswordToggle={true}
        />
