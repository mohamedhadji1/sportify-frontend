...
export const ProfileImageUpload = ({ currentImage, onImageUpdate, className = "" }) => {
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/auth/upload-profile-image`, {
        method: "POST",
        headers: {
          "x-auth-token": token,
        },
  ...
  const handleRemoveImage = async () => {
    ...
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(`${process.env.REACT_APP_API_URL}/auth/remove-profile-image`, {
        method: "DELETE",
        headers: {
          "x-auth-token": token,
        },
